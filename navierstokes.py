...
def simFlow(maxh, domain_type, nu, tau, tend):
    ...
    with TaskManager():
        while t < tend:
            ...
            Redraw()
            ngsint.SnapShot('snap_shits/' + domain_type + '/test_' + t_str)
...
def main():
    ...
    base_dir = 'snap_shits/'
    directory = base_dir + domain_type
    images = []
    filenames = sorted(os.listdir(directory))
    for filename in filenames:
        images.append(imageio.v2.imread(directory + '/' + filename))
    imageio.mimsave(domain_type + '.gif', images)
